...
class FastCopy:
    ...
    def __init__(self, src_dir: str, dest_dir: str, delete: bool = False, sync: bool = False, replace: bool = False,
                 size_limit=0, thread_num=16, ignore_symlinks=False):
        ...
        print(
            f"{len(file_list)} file(s) to copy from {self.src_dir} to {self.dest_dir} with a size of {sizeof_fmt(self.size)}")
        if sync:
            dir_delete_count = 0
            delete_count = 0
            exist_count = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-s', '--sync', help='Remove files from destination folder if they don\'t exist in source directory',
        action='store_true')
    parser.add_argument(
